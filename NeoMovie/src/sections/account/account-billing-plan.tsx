...
export default function AccountBillingPlan({ cardList, plans }: Props) {
  ...
  } else if (user.plan && user.plan.id === 0) {
  ...
  const handleUpgradePlan = () => {
    const currentPlan = plans.filter((plan) => plan.id === user.plan.id)[0].subscription;
  ...
  const renderPlans = plans.map((plan) => (
    <Grid xs={12} md={4} key={plan.subscription}>
      <Stack
        component={Paper}
        variant="outlined"
        onClick={() => handleSelectPlan(plan.subscription, plan.id)}
        sx={{
          p: 2.5,
          position: 'relative',
          cursor: 'pointer',
      ...
      <Card sx={{ backgroundColor: 'black' }}>
        ...
        <Stack spacing={2} sx={{ p: 3, pt: 0, typography: 'body2' }}>
          ...
          <Grid container spacing={{ xs: 0.5, md: 2 }}>
            ...
            <Grid xs={12} md={8}>
              <Button sx={{ typography: 'subtitle2', p: 0, borderRadius: 0 }}>
                {user.displayName}
              </Button>
            </Grid>
          </Grid>
        ...
        <Stack spacing={1.5} direction="row" justifyContent="flex-end" sx={{ p: 3 }}>
          <Button variant="outlined" onClick={handleCancelPlan}>
            Cancel Plan
          </Button>
